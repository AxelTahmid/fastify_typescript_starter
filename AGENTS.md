...
Actual layout (keep new code consistent with this):

```md
src/
  server.ts              # createServer(): fastify instance + plugin graph; listens when run directly
  routes.ts              # registers feature route modules under prefixes
  acl/                   # Permission enum, catalog, wildcard expansion, role presets (+ unit tests)
  app/                   # feature modules: <feature>/{routes,handler(s),service,repository,schema,types}.ts
    auth/                # login/refresh/logout + OTP flows, refresh-cookie helpers
    base/                # health + admin debug endpoints
    gallery/             # object storage example (direct + presigned upload)
    queue/               # ACL-guarded queue admin endpoints
  config/
    env.ts               # TypeBox-validated environment — fail-fast, requireInProd for secrets
    environment.ts       # typed AppConfig assembled from validated env
    schema.ts            # shared TypeBox models ($id envelopes)
  database/
    connection.ts        # standalone Kysely factory for CLI tools
    db.d.ts              # generated by kysely-codegen (make db-types)
    helpers.ts           # paginate, filter.*, pgerr, isPgError, health
    migrate.ts           # up|down|latest|status|reset
    migrations/          # NN_name.ts, single-responsibility
    seed-acl.ts          # reconciles code ACL catalog into DB (idempotent)
    seed-admin.ts        # ensures the env-configured admin exists
  plugins/               # db, cache, jwt, input-guard, storage, pgboss, nodemailer, schemas
  queue/                 # pg-boss: base ops class, per-job config, workers, cron scheduling
  tests/                 # integration tests + buildApp() helper (app.inject against real PG)
  utils/                 # password (argon2id), rate-limit (PG-backed)
```
...
- Classes: `PascalCase`
- Feature folders: domain-based (`src/app/<feature>/...`)
...
### 11.1 ACL & Auth Conventions

- Authorization is permission-based, never role-string checks. Guard routes with
  `onRequest: app.auth.can(Permission.X)` or `app.auth.any(...)` — default deny.
- New permissions: add the slug to `src/acl/slugs.ts` (`module.action` form), fill the catalog
  entry (compile error if you forget), grant it in `src/acl/presets.ts` where appropriate.
  `make db-seed` reconciles the DB.
- Write permissions never imply read. Only the `system.admin` / `system.viewer` wildcards expand.
- Access tokens carry expanded permissions and are short-lived; refresh tokens exist only in
  httpOnly cookies with a `jti`. Revocation goes through the cache denylist
  (`app.auth.revokeRefreshToken` / `revokeAllForUser`).
- Passwords: argon2id via `#utils/password.js` only — never import argon2 directly elsewhere.
- Abuse control: `assertRateLimit(app, key, limit, windowSeconds)` from `#utils/rate-limit.js`
  on any unauthenticated mutation; clear with `clearRateLimit` on success where it makes sense.
